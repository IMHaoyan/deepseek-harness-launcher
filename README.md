...
- **稳定性机制（v1.1.7+，借鉴 dsh-desktop 的主流做法）**：
  ...
  - **启动中状态**：面板状态行区分「正在启动服务… / 正在停止服务… / 服务正在自动重启… / 运行中 / 已停止」（`state.phase`），启动/停止期间「启动/停止服务」按钮禁用；就绪超时（60 秒）会强杀卡住的子进程并如实报失败，不再留下"进程活着但永远显示正在启动"的状态；
  - **停止防重入 + 看门狗**：停止流程由 `service-stop-guard.js` 统一置位/复位（重复点击只执行一次、不再重复弹"服务已停止"）；30 秒看门狗兜底——即使底层 `taskkill` 卡住也会强制复位状态并记日志，按钮不会永久变灰；
...
- **插件市场（v1.1.9+，设置页一行开关）**：把 DSH 官方生态里的 `dshmarket`（DSH 内的可视化插件市场）装进 web profile。
  - **安装**：设置页「插件市场」行 → 点「安装」（二次确认）→ 主进程先用 npm 官方源校验包身份（包名一致 + 精确稳定版本 + 合法 `dsh.bundle.patch`）→ 执行 `dsh plugin --profile web add dshmarket@<版本> -w`（DSH CLI 自带的 pnpm 薄封装，自动写入 `dsh.profile.bundles`；`-w` 是因为 profile 目录本身是 pnpm workspace 根）→ 二次校验 profile 的 `dependencies` 与 `bundles` → **自动重启服务生效**；已安装时按钮变「卸载」，卸载对称。
  - **默认安装**：升级到新版本后首次启动会自动装一次（每个启动器版本最多尝试一次，记账在 `pluginMarketAutoTryVersion`），装完弹通知；失败不反复重试、只记日志 + 提示可在设置页手动重试。用户在设置页**主动卸载**过则记录 `pluginMarketDeclined`，不再自动装回来。
  - **不装也不影响**：`dshmarket` 只是 DSH 内的一个插件，装/卸都不会影响 DSHL 本身；状态永远以 `~/.dsh/profiles/web/package.json` 为准（不维护安装回执）。
...
- 端口探测 + 接管：端口被占时**先做 HTTP 指纹验证**（根页面含 "DeepSeek Harness" 标题字样）确认是 DSH 才接管（退出时不停止它）；被其他程序占用则**拒绝接管与启动**：自动探测出下一个空闲端口，**自动弹出启动器面板**（主页状态卡"已停止（端口被占用）"+ 警示卡：占用端口/PID + 建议端口），**一键"换到端口 X 并启动"**；DSH 窗口的说明页同步显示冲突原因与一键换端口按钮（也可去设置页手动改端口），绝不误杀。
