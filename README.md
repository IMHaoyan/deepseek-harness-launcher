...
- 下载完成弹托盘通知；**退出重启即自动安装**，也可在设置页"版本与更新"点**重启并安装**立即更新；
- 设置页可手动"检查更新"并查看当前版本。
...
- 主题设置（浅色/深色/跟随系统）同时驱动：**启动器面板原生标题栏颜色、独立窗口 tab 栏配色、托盘图标**（经 `nativeTheme.themeSource` 落地，页面内 `prefers-color-scheme` 一并跟随）。
...
- 托盘图标自适应系统主题：深色主题显示**白色鲸鱼**、浅色显示黑色，主题切换实时跟随（`nativeTheme`）。
