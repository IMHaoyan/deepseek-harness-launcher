...
<table>
  <tr>
    ...
    <td width="50%"><img src="docs/screenshots/ui-2.png" alt="设置页：缩放、主题、端口、插件市场与 DSH 更新渠道" width="100%"></td>
  </tr>
</table>
...
- DSH：静默检测更新（24 小时节流；更新渠道可选 `latest` / `alpha`，默认 `latest`），升级需在面板点「立即更新」；更新前先停服务、更新后重启并强制重载页面，避免旧进程与新文件混用导致白屏。新版启动失败或版本不符时自动回滚到旧版。
